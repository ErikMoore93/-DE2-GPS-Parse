...
//Variables
	...
	char hour_high;          //high character of hour
	char hour_low;           //low character of hour
	int invalid_flag = 0; 
	float elev;              //elevation variable
	float degrees_latitude;  //latitude variable
	float degrees_longitude; //longitude variable
	char DN = 'A';

//Functions
//for part 12 converting time
...
int main()
{
	...
	while(1){//top while
		...
		{//scan for $ if
			...
			chk_high=getchar();
			chk_low=getchar();
			//for part 12 compute and check checksum
			char y1 = xors;
			y1 >>= 4;
			char y2 = xors;
			y2 &= 0x0F;
			...
			{//chksum if
				...
				if(gps_string[3] == 'G')
				{//GPGGA if
					switch (*Switches)//switch option for part 12
					{
						case 0x1://If switch 0 is on   //display for lat and long for part 12
						    ...
						    break;
						default: //Switch 0 is off     //display for elev and time for part 12
					...
					//convert to degrees and paste to console for part 12
				...
				else if((gps_string[3] == 'S')&&(gps_string[4]=='A'))
				{
					...
					*LEDs = 0x0;
					...
					{
						//prints satellite IDs to the console for part 13
						printf("%c",gps_string[pos]);
						//light LEDs corresponding to satellite ID numbers
						if(dum_count == 1)
						{
		...
		else //unit not ready for part 12
		{
			if (invalid_flag == 0)
			{
				fprintf(lcd, "\nGPS Unit is not ready.\n");
